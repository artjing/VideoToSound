...
namespace ofxCv {
	...
	void ContourFinder::findContours(cv::Mat img) {
        ...
        amntRectangles = boundingRects.size();
        
        // added for testing 28 08 2020
        centerPos = glm::vec2(0,0);
        for( size_t i = 0; i < boundingRects.size(); i++){
            
            centerPos += glm::vec2(static_cast<float>(boundingRects[i].x) + static_cast<float>(boundingRects[i].width) / 2,
                                  static_cast<float>(boundingRects[i].y) + static_cast<float>(boundingRects[i].height / 2));
        }
        
        if(amntRectangles != 0){
            centerPos /= static_cast<float>(amntRectangles);
        }
	}
